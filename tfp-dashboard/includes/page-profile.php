...
// Load the standalone billing shortcode so customer/reader accounts can use
// their own payment-method page without entering the Discipleship payment UI.
$tfp_customer_billing_file = dirname(__DIR__) . '/tfp-customer-billing-shortcode.php';
if (file_exists($tfp_customer_billing_file)) {
    require_once $tfp_customer_billing_file;
}
...
function tfp_dashboard_render_profile_content()
{
    ...
    if ($has_full_access) {
        $payment_details_url = function_exists('tfp_dashboard_get_url')
            ? tfp_dashboard_get_url('tfp-dashboard-payment-details')
            : '#';
    } else {
        $payment_details_url = function_exists('tfp_get_customer_billing_page_url')
            ? tfp_get_customer_billing_page_url()
            : '#';

        // Keep a safe fallback for installations where the admin has not yet
        // created the standalone page containing [tfp-dash-billing-detail].
        if (!$payment_details_url && function_exists('tfp_dashboard_get_url')) {
            $payment_details_url = tfp_dashboard_get_url('tfp-dashboard-payment-details');
        }
    }
