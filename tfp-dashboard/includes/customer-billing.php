<?php
/**
 * Standalone customer billing page.
 *
 * Shortcode: [tfp-dash-billing-detail]
 *
 * This renders only the payment-method section for e-commerce customers.
 * It intentionally does not render the Discipleship dashboard shell,
 * program banner, or program navigation.
 */
...
if (!function_exists('tfp_dashboard_render_customer_billing_shortcode')) {
    function tfp_dashboard_render_customer_billing_shortcode($atts = array())
    {
        ...
        $name    = function_exists('tfp_dashboard_user_name')
            ? tfp_dashboard_user_name()
            : ($user && $user->exists() ? $user->display_name : '');
        $card    = function_exists('tfp_billing_get_default_card_summary')
            ? tfp_billing_get_default_card_summary($user_id)
            : null;
        ...
        // These assets are loaded only when this shortcode is actually used.
        wp_enqueue_style(
            'tfp-dashboard-customer-billing',
            TFP_DASH_URL . 'assets/css/customer-billing.css',
            array(),
            defined('TFP_DASH_VERSION') ? TFP_DASH_VERSION : null
        );
        ...
        if ($stripe_ready) {
            ...
            wp_enqueue_script(
                ...
                TFP_DASH_URL . 'assets/js/billing.js',
                array('stripe-js'),
                defined('TFP_DASH_VERSION') ? TFP_DASH_VERSION : null,
                true
            );
            wp_localize_script('tfp-dashboard-billing', 'tfpDashboardBilling', array(
        ...
        <section class="tfp-customer-billing" aria-labelledby="tfp-customer-billing-title">
            <div class="tfp-customer-billing__intro">
                <h2 id="tfp-customer-billing-title">
                    <?php printf(esc_html__('Hi, %s — Update payment method', 'tfp-dashboard'), esc_html($name)); ?>
                </h2>
            ...
            <div class="tfp-customer-billing__grid">
                <div class="tfp-customer-billing__preview">
                    <div class="tfp-billing-visual-card-wrap">
                        <div
                            class="tfp-billing-visual-card is-<?php echo esc_attr($card_brand); ?>"
                            data-tfp-visual-card
                            data-card-brand="<?php echo esc_attr($card_brand); ?>"
                        >
                            <div class="tfp-billing-visual-card__top">
                                <span class="tfp-billing-visual-card__type" data-tfp-visual-card-type>
                                    <?php echo esc_html($card_brand === 'generic' ? 'Payment' : 'Credit'); ?>
                                </span>
                                <span class="tfp-billing-visual-card__brand" data-tfp-visual-card-brand>
                                    <?php echo esc_html($card_brand === 'generic' ? 'CARD' : strtoupper($card_brand)); ?>
                                </span>
                            </div>
                            <div class="tfp-billing-visual-card__chip" aria-hidden="true"></div>
                            <div class="tfp-billing-visual-card__number" data-tfp-visual-card-number>
                                <?php echo esc_html('•••• •••• •••• ' . $last4); ?>
                            </div>
                            <div class="tfp-billing-visual-card__bottom">
                                <span class="tfp-billing-visual-card__name" data-tfp-visual-card-name>
                                    <?php echo esc_html($card_name); ?>
                                </span>
                                <span class="tfp-billing-visual-card__expiry" data-tfp-visual-card-expiry>
                ...
                <div class="tfp-customer-billing__form-wrap">
                    <?php if ($card) : ?>
                        <div class="tfp-customer-billing__saved-card">
                            ...
                            <strong data-tfp-card-brand>
                                <?php printf('%s ending in %s', esc_html($card['brand']), esc_html($card['last4'])); ?>
                            </strong>
                            <small data-tfp-card-expiry><?php echo esc_html($card['expiry']); ?></small>
                        </div>
                    <?php else : ?>
                        <p class="tfp-customer-billing__no-card" data-tfp-no-card>
                            <?php esc_html_e('No payment method saved yet.', 'tfp-dashboard'); ?>
                        </p>
                    <?php endif; ?>
                        ...
                        <form class="tfp-customer-billing__form" data-tfp-billing-form novalidate>
                            ...
                            <label>
                                ...
                                <input
                                    ...
                                    value="<?php echo esc_attr($saved_name ?: $name); ?>"
                                    autocomplete="cc-name"
                                    placeholder="<?php esc_attr_e('Name as it appears on card', 'tfp-dashboard'); ?>"
                                >
                            </label>
                            <label class="tfp-customer-billing__consent">
                            ...
                            <button type="submit" class="tfp-customer-billing__submit" data-tfp-billing-submit>
                                <?php esc_html_e('Save Payment Method', 'tfp-dashboard'); ?>
                            </button>
                            <p class="tfp-dash-form__status" data-tfp-billing-status role="status"></p>
                        </form>
                    <?php else : ?>
                        <p class="tfp-customer-billing__notice" data-state="error">
                            <?php esc_html_e('Secure card updates are not configured yet. Please contact support.', 'tfp-dashboard'); ?>
                        </p>
                    <?php endif; ?>
                </div>
            </div>
...
add_shortcode('tfp-dash-billing-detail', 'tfp_dashboard_render_customer_billing_shortcode');
