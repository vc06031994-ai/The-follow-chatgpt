(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // --- Meeting Test Missed Questions Client-side Tab Switcher (No Reload) ---
        var missedNavBtns = document.querySelectorAll('.tfp-week__meeting-test-nav-item');
        var missedPanels = document.querySelectorAll('.tfp-week__meeting-test-question-area');

        if (missedNavBtns.length > 0 && missedPanels.length > 0) {
            missedNavBtns.forEach(function (btn) {
                btn.addEventListener('click', function (e) {
                    e.preventDefault();
                    var targetIndex = btn.getAttribute('data-missed-target');

                    missedNavBtns.forEach(function (b) { b.classList.remove('is-active'); });
                    btn.classList.add('is-active');

                    missedPanels.forEach(function (p) {
