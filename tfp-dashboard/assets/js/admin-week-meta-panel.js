(function (wp) {
    ...
    var FIELDS = [
        ...
        { key: 'tfp_week_meeting_exercise_description', label: __('Exercise Description', 'tfp-dashboard'), placeholder: 'Write the exercise description shown to students', type: 'textarea' },
        { key: 'tfp_week_meeting_exercise_purpose', label: __('Exercise Purpose', 'tfp-dashboard'), placeholder: 'Write the exercise purpose / intent', type: 'textarea' },
    ];
    ...
    var TfpWeekPanel = compose(
        ...
        // Debug: ensure this script executed in the block editor.
        try { console.log('TFP Week Details panel script loaded', props && props.meta ? Object.keys(props.meta) : null); } catch (e) {}
        var meta = props.meta || {};
        ...
        // Structured Facilitator Notes editor (JSON list)
        var notesRaw = meta['tfp_week_meeting_facilitator_notes_list'] || '';
        var parseNotes = function (json) {
            if (!json) return [];
            try { var d = JSON.parse(json); if (Array.isArray(d)) return d; } catch (e) {}
            return [];
        };

        var notesList = parseNotes(notesRaw);
        var notesState = useState(function () { return notesList; });
        var notes = notesState[0];
        var setNotes = notesState[1];

        var newNoteState = useState('');
        var newNote = newNoteState[0];
        var setNewNote = newNoteState[1];

        function persistNotes(list) {
            setNotes(list);
            props.setMeta(buildMeta(meta, 'tfp_week_meeting_facilitator_notes_list', list.length ? JSON.stringify(list) : ''));
        }

        function addNoteFromEditor() {
            var text = (newNote || '').trim();
            if (!text) return;
            var author = (window.tfpWeekEditorSettings && window.tfpWeekEditorSettings.currentUserId) ? window.tfpWeekEditorSettings.currentUserId : 0;
            var entry = { author_id: author, text: text, created_at: new Date().toISOString() };
            var updated = (notes || []).slice();
            updated.unshift(entry); // newest first
        ...
        // Render facilitator notes editor block
        var notesNodes = [];
        notesNodes.push(el('strong', { key: 'notes-title', className: 'tfp-hw-title' }, __('Facilitator Notes (Structured)', 'tfp-dashboard')));
        notesNodes.push(el('p', { key: 'notes-help', className: 'tfp-hw-help' }, __('Add short notes that appear in the Meeting → Notes panel. New notes appear at the top.','tfp-dashboard')));

        if (notes && notes.length) {
            notes.forEach(function (n, i) {
                var authorName = n.author_id ? ('User ' + n.author_id) : __('Facilitator','tfp-dashboard');
                var time = n.created_at ? new Date(n.created_at).toLocaleString() : '';
                notesNodes.push(
                    el('div', { key: 'note-' + i, className: 'tfp-fac-note' },
                        el('div', { className: 'tfp-fac-note-meta' }, el('strong', null, authorName), ' ', el('span', { className: 'tfp-fac-note-time' }, time)),
                        el('div', { className: 'tfp-fac-note-text' }, n.text),
                        el('div', { className: 'tfp-fac-note-actions' }, el(Button, { isSmall: true, isDestructive: true, onClick: function () { removeNote(i); } }, __('Delete', 'tfp-dashboard')))
                    )
                );
            });
        } else {
            notesNodes.push(el('p', { key: 'notes-empty', className: 'tfp-hw-empty' }, __('No structured notes yet — use the form below to add one.', 'tfp-dashboard')));
        }

        notesNodes.push(el(TextareaControl, {
            key: 'new-note',
            label: __('New Facilitator Note', 'tfp-dashboard'),
            value: newNote,
            rows: 3,
            onChange: function (value) { setNewNote(value); }
        }));

        notesNodes.push(el(Button, { key: 'notes-add', isPrimary: true, onClick: addNoteFromEditor }, __('Add Note', 'tfp-dashboard')));

        elements.push(el('div', { key: 'notes-section', className: 'tfp-hw-section' }, notesNodes));
